let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    if (args.length < 3) {
        return conn.reply(m.chat, `Gunakan format .mentransfer <type> <jumlah> <@tag>\nðŸ“contoh penggunaan: *.mentransfer money 100 @tag*\n\n*List yang bisa di transfer :*\nðŸ’¹Money\nðŸ’³ Tabungan\nðŸ¥¤Potion\nðŸ—‘ï¸Sampah\nðŸ’ŽDiamond\nðŸ“¦Common\nðŸ›ï¸Uncommon\nðŸŽMythic\nðŸ§°Legendary\nðŸ•¸ï¸string\nðŸªµkayu\nðŸª¨batu\nâ›“iron`.trim(), m)
    } else try {
        let type = (args[0] || '').toLowerCase()
        let count = args[1] && args[1].length > 0 ? Math.min(, Math.max(parseInt(args[1]), 1)) : Math.min(1)
        ...
        switch (type) {
            ...
            case 'iron':
                ...
                break
            default:
                return conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\nðŸ“ Contoh penggunaan: *${usedPrefix}transfer money 100 @tag*\n\n*List yang bisa di transfer*\nðŸ’¹ Money\nðŸ’³ Tabungan\nðŸ¥¤ Potion\nðŸ—‘ï¸ Sampah\nðŸ’Ž Diamond\nðŸ“¦ Common\nðŸ›ï¸ Uncommon\nðŸŽ Mythic\nðŸ§° Legendary\nðŸ•¸ï¸ String\nðŸªµ Kayu\nðŸª¨ Batu\nâ›“ï¸ Iron`.trim(), m)
        }
    } catch (e) {
        conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\ðŸ“ Contoh penggunaan: *${usedPrefix}transfer money 100 @tag*\n\n*List yang bisa di transfer :*\nðŸ’¹ Money\nðŸ’³ Tabungan\nðŸ¥¤ Potion\nðŸ—‘ï¸ Sampah\nðŸ’Ž Diamond\nðŸ“¦ Common\nðŸ›ï¸ Uncommon\nðŸŽ Mythic\nðŸ§° Legendary\nðŸ•¸ï¸ String\nðŸªµ Kayu\nðŸª¨ Batu\nâ›“ iron`.trim(), m)
        console.log(e)
        if (DevMode) {
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                conn.reply(jid, 'Transfer.js error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', m)
...
handler.help = ['tfown <Args>']
handler.tags = ['rpg']
handler.command = /(tfown)$/i
handler.owner = true
handler.mods = false
